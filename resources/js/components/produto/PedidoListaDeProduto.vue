<template>
    <div class="lista">     
        <table class="table">
            <thead>
                <tr>
                    <th><a href="#" @click="sortNumero('id')">Código</a></th>
                    <th><a href="#" @click="sortUsers('nome')">Nome</a></th>
                    <th><a href="#" @click="sortNumero('codbarras')">Código de barras</a></th>
                    <th><a href="#" @click="sortNumero('acrescimo')">Acréscimo(%)</a></th>
                    <th><a href="#" @click="sortNumero('desconto')">Desconto(%)</a></th>
                    <th><a href="#" @click="sortNumero('qtde')">Qtde</a></th>
                    <th><a href="#" @click="sortNumero('valor')">Valor(UN)</a></th>
                    <th><a href="#" @click="sortNumero('valor')">valor Total</a></th>
                    <th>Ação</th>
                </tr>
            </thead>    
            <draggable
                ...
                >
                    <tr v-for="(produto, i) in produtos" :key="i">
                        <td>{{produto.id}}</td>
                        <td>{{produto.nome}}</td>
                        <td>{{ Number(produto.codbarras) }}</td>
                        <td>% {{Number(produto.acrescimo) | colocarvirgula }}</td>
                        <td>% {{Number(produto.desconto) | colocarvirgula }}</td>
                        <td>{{Number(produto.qtde) | colocarvirgula }}</td>
                        <td>{{Number(produto.valor) | colocarvirgula }}</td>
                        <td>{{  valorTotalUn[i] | colocarvirgula }}</td>
...
export default {
    ...
    methods: {                   
        ...
        sortNumero(chave){
          return   this.$store.state.produtos.sort(sortBy(chave))  
        }
    }
}
</script>
